...
<template>
    ...
    >
        <header id="hero" class="relative flex h-screen w-full items-center justify-center">
            ...
            <div v-if="pending">
                <span class="z-20 animate-pulse text-4xl font-thin text-stone-100 blur-lg md:text-6xl xl:text-8xl">Loading...</span>
            </div>
            <div class="relative flex h-screen w-full items-center justify-center" v-else>
                <h1 class="z-20 text-4xl font-thin text-stone-100 transition-all duration-200 ease-in-out md:text-6xl xl:text-8xl">
                    {{ project.name }}
                </h1>
        ...
        <main id="info" class="flex w-full flex-col gap-7 p-7 md:p-12 xl:px-24 2xl:px-48">
            <ul class="flex flex-col gap-7">
                <li>
                    ...
                    <span class="animate-pulse text-sm blur" v-if="pending">
                        Lorem ipsum dolor sit, amet consectetur adipisicing elit. Commodi aperiam deleniti illum, nulla, error repudiandae quaerat beatae ullam facilis, dolore nam accusamus aliquid
                        vitae. Illum eveniet illo corrupti quaerat, nostrum necessitatibus esse soluta ratione quia exercitationem dolorem modi fuga incidunt architecto? Voluptate dolores beatae
                        mollitia, quam hic aspernatur delectus nesciunt?
                    </span>
                    <p class="whitespace-pre-wrap text-sm" v-else>{{ project.long_description }}</p>
                </li>
                <li>
                    <h3 class="text-xl font-bold">Links</h3>
                    <div class="flex flex-wrap gap-2 text-sm">
                        <span class="animate-pulse underline blur" v-if="pending">Lorem ipsum dolor sit</span>
                        <a class="underline" v-else :href="link.url" v-for="link in project.links">{{ link.name }}</a>
                    </div>
                </li>
            </ul>
            <ul class="flex flex-wrap justify-between gap-7 md:-order-1">
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <span class="animate-pulse text-sm blur" v-if="pending">HEY, SUP?</span>
                    <p class="text-sm" v-else>{{ project.category }}</p>
                </li>
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <span class="animate-pulse text-sm blur" v-if="pending">1377</span>
                    <p class="text-sm" v-else>{{ project.visits === null ? 'N/A' : new Intl.NumberFormat().format(project.visits) }}</p>
                </li>
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <span class="animate-pulse text-sm blur" v-if="pending">69</span>
                ...
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <span class="animate-pulse text-sm blur" v-if="pending">I'm poor</span>
                    <p class="text-sm" v-else>
                ...
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <span class="animate-pulse text-sm blur" v-if="pending">0</span>
                    <p class="text-sm" v-else>
                ...
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <span class="animate-pulse text-sm blur" v-if="pending">0</span>
                    <p class="text-sm" v-else>
                        {{ project.youtube.comments === null ? 'N/A' : new Intl.NumberFormat().format(project.youtube.comments) }}
                    </p>
                </li>
            </ul>
            <ul class="flex flex-wrap justify-between gap-7">
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <div class="flex flex-wrap gap-2 text-sm">
                        <span class="animate-pulse text-sm blur" v-if="pending">Hmmm, its empty here...</span>
                        <p v-for="tech in project.tech_stack" v-else>{{ tech }}</p>
                    </div>
                </li>
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <span class="animate-pulse text-sm blur" v-if="pending">1986-04-25</span>
                    <p class="text-sm" v-else>{{ project.github.created_at === null ? 'N/A' : project.github.created_at }}</p>
                </li>
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <span class="animate-pulse text-sm blur" v-if="pending">01:23:40</span>
                    <p class="text-sm" v-else>{{ project.github.updated_at === null ? 'N/A' : project.github.updated_at }}</p>
                </li>
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <div class="flex flex-wrap gap-2 text-sm">
                        <span class="animate-pulse text-sm blur" v-if="pending">also, no links here</span>
                        <p v-else-if="project.github.licenses === null">N/A</p>
                        <a class="underline" v-else :href="license.url" v-for="license in project.github.licenses">{{ license.name }}</a>
                    </div>
                </li>
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <span class="animate-pulse text-sm blur" v-if="pending">large</span>
                    <p class="text-sm" v-else>{{ project.github.size === null ? 'N/A' : project.github.size }} {{ project.github.size_unit }}</p>
                </li>
                <li class="flex flex-col items-center md:items-start">
            ...
            <ul class="flex w-full flex-col flex-wrap items-center justify-center gap-7 md:flex-row md:justify-evenly">
                <li v-if="!pending" v-for="asset in project.assets" v-motion-pop-visible-once>
                ...
                <span v-if="pending" class="aspect-video w-full animate-pulse rounded-lg bg-slate-200 dark:bg-zinc-800 lg:w-80"></span>
                <span v-if="pending" class="aspect-video w-full animate-pulse rounded-lg bg-slate-200 dark:bg-zinc-800 lg:w-80"></span>
                <span v-if="pending" class="aspect-video w-full animate-pulse rounded-lg bg-slate-200 dark:bg-zinc-800 lg:w-80"></span>
            </ul>
        </main>
