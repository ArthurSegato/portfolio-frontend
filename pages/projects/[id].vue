...
<template>
    ...
    >
        <header id="hero" class="relative flex h-screen w-full items-center justify-center">
            ...
            <img
                class="absolute top-0 h-full w-full select-none object-cover object-center"
                v-if="isFinished && data.cover.mimetype === 'image/avif'"
                :src="data.cover.url"
            />
            <video
                autoplay
                ...
                loop
                class="absolute top-0 h-full w-full select-none object-cover object-center"
                v-if="isFinished && data.cover.mimetype === 'video/mp4'"
            >
                <source :src="data.cover.url" :type="data.cover.mimetype" />
            </video>
            <h1 class="z-20 text-4xl font-thin text-stone-100 transition-all duration-200 ease-in-out md:text-6xl xl:text-8xl" v-if="isFinished">
                {{ data.name }}
            </h1>
        ...
        <main id="info" class="flex w-full flex-col gap-7 p-7 md:p-12 xl:px-24 2xl:px-48">
            <ul class="flex flex-col gap-7">
                <li>
                    ...
                    <span class="animate-pulse text-sm blur" v-else>
                        Lorem ipsum dolor sit, amet consectetur adipisicing elit. Commodi aperiam deleniti illum, nulla, error repudiandae quaerat
                        beatae ullam facilis, dolore nam accusamus aliquid vitae. Illum eveniet illo corrupti quaerat, nostrum necessitatibus esse
                        soluta ratione quia exercitationem dolorem modi fuga incidunt architecto? Voluptate dolores beatae mollitia, quam hic
                        aspernatur delectus nesciunt?
                    </span>
                </li>
                <li>
            ...
            <ul class="flex flex-wrap justify-between gap-7 md:-order-1">
                ...
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <p class="text-sm" v-if="isFinished">{{ data.visits === null ? 'N/A' : new Intl.NumberFormat().format(data.visits) }}</p>
                ...
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <p class="text-sm" v-if="isFinished">{{ data.downloads === null ? 'N/A' : new Intl.NumberFormat().format(data.downloads) }}</p>
                ...
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <p class="text-sm" v-if="isFinished">{{ data.revenue === null ? 'N/A' : `$ ${new Intl.NumberFormat().format(data.visits)}` }}</p>
                ...
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <p class="text-sm" v-if="isFinished">
                        {{ data.youtube.views === null ? 'N/A' : new Intl.NumberFormat().format(data.youtube.views) }}
                    </p>
                ...
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <p class="text-sm" v-if="isFinished">
                        {{ data.youtube.likes === null ? 'N/A' : new Intl.NumberFormat().format(data.youtube.likes) }}
                    </p>
                    <span class="animate-pulse text-sm blur" v-else>0</span>
                </li>
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <p class="text-sm" v-if="isFinished">
                        {{ data.youtube.comments === null ? 'N/A' : new Intl.NumberFormat().format(data.youtube.comments) }}
                    </p>
                    <span class="animate-pulse text-sm blur" v-else>0</span>
                </li>
            </ul>
            <ul class="flex flex-wrap justify-between gap-7">
                ...
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <p class="text-sm" v-if="isFinished">{{ data.github.created_at === null ? 'N/A' : data.github.created_at }}</p>
                ...
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <p class="text-sm" v-if="isFinished">{{ data.github.updated_at === null ? 'N/A' : data.github.updated_at }}</p>
                ...
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <div class="flex flex-wrap gap-2 text-sm">
                        ...
                        <p v-if="isFinished && data.github.licenses === null">N/A</p>
                    </div>
                </li>
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <p class="text-sm" v-if="isFinished">{{ data.github.size === null ? 'N/A' : data.github.size }} {{ data.github.size_unit }}</p>
                ...
                <li class="flex flex-col items-center md:items-start">
                    ...
                    <p class="text-sm" v-if="isFinished">
                        {{ data.github.stars === null ? 'N/A' : new Intl.NumberFormat().format(data.github.stars) }}
                    </p>
                    <span class="animate-pulse text-sm blur" v-else>0</span>
                </li>
            </ul>
            <ul class="flex w-full flex-col flex-wrap items-center justify-center gap-7 md:flex-row md:justify-evenly">
                <li v-if="isFinished" v-for="asset in data.assets" v-motion-pop-visible-once>
                    <img
                        class="aspect-video w-full rounded-lg object-cover object-center lg:w-80"
                        :src="asset.url"
                        :alt="asset.alt"
                        v-if="asset.mimetype === 'image/avif'"
                    />
                    <video autoplay muted loop class="aspect-video w-full rounded-lg lg:w-80" v-else>
                        <source :src="asset.url" :type="asset.mimetype" />
                    </video>
                </li>
                <span v-if="!isFinished" class="aspect-video w-full animate-pulse rounded-lg bg-slate-200 dark:bg-zinc-800 lg:w-80"></span>
                <span v-if="!isFinished" class="aspect-video w-full animate-pulse rounded-lg bg-slate-200 dark:bg-zinc-800 lg:w-80"></span>
                <span v-if="!isFinished" class="aspect-video w-full animate-pulse rounded-lg bg-slate-200 dark:bg-zinc-800 lg:w-80"></span>
            </ul>
        </main>
